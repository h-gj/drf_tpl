...
def custom_exception_handler(exc, context):
    ...
    if isinstance(exc, ValidationError):
        ...
        return JsonResponse({'code': 2000, 'message': friendly_msg, 'data': {}})

    elif isinstance(exc, APIException):
        return JsonResponse({'code': exc.get_codes(), 'message': exc.detail, 'data': {}})

    elif isinstance(exc, Http404):
        return JsonResponse({'code': 2004, 'message': 'Route not found.', 'data': {}})

    else:
        return JsonResponse({'code': 3000, 'message': exc.__str__() or '服务器开小差了～', 'data': {}})
